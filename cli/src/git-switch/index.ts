...
async function tryLogin(retryCount = 3){
    try{
        ...
        runShell(`gh auth login --hostname github.com --with-token < ${tempFileName}`, null, false, true);
        ...
        if(retryCount <= 0){
            console.log(chalk.red('please start again and this time type the correct personal access token!'));
            process.exit(1);
        }

        if(stderr?.includes('missing required scope')){
            config.user.token = await askAndSetConfig(`the personal access token of "${config.user.name}" doesn't have required scopes: "repo", "read:org". please generate a new one and type it here.`, `${nextAccountKey}.token`);
            if(retryCount > 0) 
                return tryLogin(--retryCount)
        }else if(stderr?.includes('Bad credentials')){
            config.user.token = await askAndSetConfig(`the personal access token of "${config.user.name}" is either expired or wrong. please generate a new one and type it here.`, `${nextAccountKey}.token`);
            if(retryCount > 0) 
                return tryLogin(--retryCount)
        }else{
            throw err;
...
if(await tryLogin()){
    rmSync(tempFileName);
    console.log(chalk.green(`successfully switched from "${lastUserName}" to "${config.user.name}"`));
    process.exit(0);
}
process.exit(2);